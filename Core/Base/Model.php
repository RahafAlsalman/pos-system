...
class Model
{
    ...
    public function get_transaction_value(): array
    {
        $data = array();
        $result = $this->connection->query("SELECT * FROM $this->table");
    ...
    public function sum()
    {
        $result = $this->connection->query("SELECT  SUM(total) AS total_seales FROM transactions");
        $row = $result->fetch_array();
                $data = $row['total_seales'];
    ...
    public function update($data)
    {
        ...
        SET $set_values WHERE id=?"); // prepare the sql statement
        $stmt->bind_param('i', $id); // bind the params per data type
        $stmt->execute(); // execute the statement on the DB
        $result = $stmt->get_result(); // get the result of the execution
        $stmt->close();
